...
class ASR(sb.core.Brain):
    def compute_forward(self, batch, stage):
        ...
        if hasattr(self.hparams, "train_mode"):
            ...
            if mode == 0 or mode == 2:
                pred = self.hparams.Transformer(tokens_bos)
                # print( f"My lovely batch x length x dim :  {pred.shape}" )
                p_ctc = None
                wav_lens = None
            else:
    ...
    def evaluate_batch(self, batch, stage):
        ...
        with torch.no_grad():
            ...
            if predictions[0] != None:
                ...
                loss = self.compute_objectives(predictions, batch, state = stage)
            else:
                # LM loss, use 
                loss = self.compute_lm_objectives(predictions[1], batch, stage)
    ...
    def fit_batch(self, batch):
        ...
        if self.auto_mix_prec:
            ...
            if outputs[0] != None:
                ...
                # LM loss, use 
                loss = self.compute_lm_objectives(outputs[1], batch, sb.Stage.TRAIN)

            if self.log_to_wandb:
                wandb.log({"loss": loss.item(), "nonfinite_count": self.nonfinite_count})
...
if __name__ == "__main__":
    ...
    if int(hparams["train_mode"]) in [0,2] :
        # LM mode or co-training, change csv name
        a = hparams["train_csv"].replace('train.csv', 'train_lm.csv')
        print("\n\nI will replace the file name for name, only LM mode (0)\nfor co-train, need more changes\n\n")
        hparams["train_csv"] = a 
